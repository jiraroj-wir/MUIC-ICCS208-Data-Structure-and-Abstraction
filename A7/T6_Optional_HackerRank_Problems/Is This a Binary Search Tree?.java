...
public class Tree {
    ...
    */
    boolean checkBST(Node root) { return validate(root, 0, 10000); }

    private static boolean validate(Node root, int floor, int sealing) {
        if (root.left == null && root.right == null) {
            return true;
        }

        if (root.left.data >= root.data || root.right.data <= root.data || root.left.data <= floor ||
            root.right.data >= sealing) {
            return false;
        }

        return validate(root.left, floor, root.data) && validate(root.right, root.data, sealing);
    }
