...
For this task, save your work in \texttt{smallman/Midway.java}. Monks at a remote monastery are busy solving the Tower of Hanoi problem, a duty passed on for tens of generations. According to an old tale, when this group of monks finishes, $P$ will be shown to equal $NP$, and the world will come to an end.
...
\textbf{Subtask II:} Implement
\begin{verbatim}
public static long stepsRemaining(int[] diskPos)
\end{verbatim}
that takes in the current positions of the disks and returns the number of steps that remain in the computer-generated instruction sequence. The current positions are given as an array of integers: \texttt{diskPos.length} tells you how many disks there are, and the entry at index $i$ (written \texttt{diskPos[i]}) is one of $\{0, 1, 2\}$ indicating the peg where Disk $i$ resides. We guarantee $0 \le \texttt{diskPos.length} \le 63$. For example:
\begin{itemize}
    \item \texttt{stepsRemaining(\{0\})} should return $1$.
    \item \texttt{stepsRemaining(\{2, 2, 1\})} should return $3$.
    \item \texttt{stepsRemaining(\{2, 2, 1, 1, 2, 2, 1\})} should return $51$.
\end{itemize}
(Hint: Solve the inputs \texttt{\{2, 2, 0\}} and \texttt{\{1, 2, 0\}} by hand. How many moves occur before the largest disk moves?)

\textbf{Performance Expectations:} Your code should return within 1 second and use only a reasonable amount of memory (e.g., do not explicitly generate the entire instruction sequence).
